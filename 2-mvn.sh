...
MVN_SEPARATOR=".*\-\-\-.*|webapp|war|module|classe"
...
# '\ in\ '|'but was'|
#
MVN_KEYWORD_FAIL="fake|SMTP|\->|unchecked|unsafe|problems|missing|deprecated|assertionerror|errors|error|failure!|failures|failure|failed|onfail|fail|fatal|cannot|not|unable|null|java\.util\.NoSuchElementException|NoSuchElement|Exception|java.net.BindException:"  #|'Address already in use'
...
alias   c='mvn clean install -U -P Atnv2_Local,gwt-dev,resources-dev -DskipTests                                          2>&1        |  h -i $MVN_ALL'
alias  ct='mvn clean install -U -P Atnv2-TU,gwt-dev,resources-dev        2>/dev/null    | grep -v "DATABASECHANGELOG|executed|changeset|expected\ postgresql,\ got\ h2|Successfully\ released\ change\ log\ lock|Successfully\ acquired\ change\ log\ lock"   |  h -i $MVN_ALL'
alias  cv='mvn clean install -U -P Atnv2-TU,gwt-dev,resources-dev        2>/dev/null    | grep -v "DATABASECHANGELOG|executed|changeset|expected\ postgresql,\ got\ h2|Successfully\ released\ change\ log\ lock|Successfully\ acquired\ change\ log\ lock"   |  h -i $MVN_ALL'
